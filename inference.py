...
1. Video Processing - Call tennis_analysis pipeline for player/ball tracking
...
def main():
    ...
    try:
        # STEP 1: Video Processing using tennis_analysis pipeline
        ...
        player_2 = analysis_results["players"]["player_2"]
        total_shots = analysis_results["ball_tracking"]["total_shots"]
        
        biomechanics = {
            "kinetic_chain_efficiency": 78,  # Placeholder - would need pose analysis
            "core_rotation_speed": 145,
            "shoulder_hip_separation": 42,
            "racket_head_speed": max(player_1["avg_shot_speed_kmh"], player_2["avg_shot_speed_kmh"]) * 0.621,  # km/h to mph
            "balance_score": 82,
            "footwork_efficiency": 75,
            "stroke_consistency": 71,
            "detected_strokes": [
                {"type": "forehand", "count": player_1["shots"], "avg_quality": 0.76},
                {"type": "backhand", "count": player_2["shots"], "avg_quality": 0.68}
            ],
            "key_moments": [],
            "player_stats": {
                "player_1": player_1,
            ...
            "distance_traveled": {
                "player_1": player_1["distance_traveled_m"],
                "player_2": player_2["distance_traveled_m"]
            }
        }

        # STEP 2: Annotated Video (already generated by tennis_analysis)
        print("\n[STEP 2] ANNOTATED VIDEO GENERATION")
        print("-" * 40)
        print(f"[STEP 2] Annotated video already generated: {annotated_path}")
