...
class MiniCourt():
   ...
   def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
       ...
       for frame_num, player_bbox in enumerate(player_boxes):
           ...
           for player_id, bbox in player_bbox.items():
               ...
               # Only include frames where this player_id exists
   ...
   def get_baseline_centers(self):
    """
    Returns baseline center points in MINI-COURT pixel space
    { player_id : (x, y) }
    """

    # Horizontal center of mini court
    center_x = int(self.court_start_x + self.court_drawing_width / 2)

    # Player 1 = bottom baseline
    p1_y = int(self.court_end_y)

    # Player 2 = top baseline
    p2_y = int(self.court_start_y)
